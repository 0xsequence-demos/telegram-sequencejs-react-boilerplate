...
export const onRequest: PagesFunction<IEnv> = async (ctx) => {
  ...
  const r: {ok:boolean} = await (await fetch(apiUrl(ctx.env.BOT_TOKEN, 'setWebhook', { url: webhookUrl, secret_token: ctx.env.BOT_SECRET }))).json()
 ...
 */
function apiUrl (botToken:string, methodName:string, params:Record<string, string>) {
  let query = ''
  if (params) {
