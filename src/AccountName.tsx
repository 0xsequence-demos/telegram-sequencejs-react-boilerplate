...
import { psuedorandomName } from "./utils/randomName";
...
export function AccountName(props: { acc: Account }) {
  ...
  if (acc.type === IdentityType.Guest) {
    const i = parseInt(acc.id.replace("Guest:", "").slice(0, 14));
    return <>Guest account {psuedorandomName(i, i - 12345, i - 256739)}</>;
  }

  if (acc.type === IdentityType.PlayFab) {
    return <>PlayFab ({acc.email})</>;
  }

  if (acc.type === IdentityType.OIDC) {
    if (acc.issuer?.includes("cognito-idp")) {
      return <>Email Legacy ({acc.email})</>;
    }
  }
  ...
  if (
    import.meta.env.VITE_STYTCH_LEGACY_ISSUER &&
    acc.issuer === import.meta.env.VITE_STYTCH_LEGACY_ISSUER
  ) {
